...
users_table <- combined_data %>% 
  ...
  select(user_id, user_first_name, user_last_name, user_email, user_password, user_mobile_number, user_membership_status, address_city, address_country, address_state, address_postcode, address_type)
write.csv(users_table, "users_table.csv", row.names = FALSE)

# 2. Addresses Table
#addresses_table <- combined_data %>%
#  distinct(address_id, .keep_all = TRUE) %>%
#  select(address_id, user_id, address_city, address_country, address_state, address_postcode, address_type)
#write.csv(addresses_table, "addresses_table.csv", row.names = FALSE)

# 3. Products Table
products_table <- combined_data %>%
  distinct(product_id, .keep_all = TRUE) %>%
  select(product_id, product_description, product_code, category_id, product_stock, product_price, product_name, product_brand, product_weight, product_dimensions, product_published_datetime, product_average_star_ratings, product_reviews, seller_id)
...
order_details_table <- combined_data %>%
  select(order_detail_id, category_id, product_id, order_qty, order_price, order_status, order_datetime, user_id, payment_method)
...
deliveries_table <- combined_data %>%
  ...
  select(delivery_id, delivery_type, delivery_status, delivery_datetime, user_id, order_detail_id, shipper_id, product_id)
